...
class PackageController extends Controller
{
    ...
    {
        $this->middleware(['stockist', 'auth']);

    }


    public function stockist_member_upgrade()
    {
        //        dd($package);
        $packages = Package::all();

        return view('stockists.packages.change', compact('packages'));
    }


    public function save_member_upgrade(Request $request)
    {
        $request->validate([
...
//            echo 'STOCKIST BAL:'. $stockist_reward->stockst;
//            echo 'OLD PACKAGE BONUS: '.$old_package->amount.' B: '. $old_bonus . '<br>';
//            echo 'NEW PACKAGE AMOUNT: '.$new_package->amount .' B:'. $new_bonus . '<br>';
...
//            echo 'PACKAGE DIFFERENCE: '. $package_diff . '<br>';
...
//                dd($user->package_id, (int)$request->new_package);
            ...
            if ($stockist_reward->stockist < $package_diff)
            {
//               dd($stockist_reward->cash, $stockist_reward->stockist);
                ...
                if ($stockist_reward->stockist >= $package_diff)
                {
                    $stockist_reward->stockist = $stockist_reward->stockist - $package_diff;
                    $stockist_reward->save();
...
//            echo 'STOCKIST NEW BAL: '.$stockist_reward->stockist .'<br>';
...
//
//            echo 'STOCKIST OLD BAL: '.$stockist_reward->stockist .'<br>';
                    $transaction = new TransactionController();
                    $transaction->log_transaction('DEBIT', $package_diff, 'PACKAGE_UPGADE', $stockist->username,
                        ['upgraded'=> $user->username,
                            'from' => $old_package->name,
                            'to' => $new_package->name
                        ]);
